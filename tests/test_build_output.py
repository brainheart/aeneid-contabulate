"""Sanity checks on the generated the Aeneid build output."""
...
class TestWorks(unittest.TestCase):
    ...
    def test_has_12_works(self):
        self.assertEqual(len(self.works), 12)
    ...
    def test_genre_counts(self):
        ...
        self.assertEqual(counts['Epic'], 12)
    ...
    def test_locations_follow_catalog_order(self):
        self.assertEqual(self.works[0]["location"], "01.Aen")
...
class TestParagraphs(unittest.TestCase):
    ...
    def test_paragraphs_are_present(self):
        self.assertEqual(len(self.paragraphs), 9896)

    def test_first_paragraph_shape(self):
        first = self.paragraphs[0]
        self.assertEqual(first["canonical_id"], "Aen.1.1")
        self.assertEqual(first["location"], "01.Aen.001.0001")
...
class TestTokens(unittest.TestCase):
    ...
    def test_token_index_size(self):
        self.assertGreater(len(self.tokens), 15000)

    def test_aeneid_vocabulary_present(self):
        for word in ('arma', 'aeneas', 'virumque', 'troiae'):
            self.assertIn(word, self.tokens)
        self.assertGreater(len(self.tokens["aeneas"]), 100)
...
class TestPublishedMetadata(unittest.TestCase):
    def test_hapax_counts_and_instance_json(self):
        ...
        self.assertEqual(instance["id"], "aeneid")
        self.assertEqual(instance["stats"]["texts"], 12)
        self.assertEqual(instance["stats"]["segments"], 9896)
        self.assertEqual(instance["stats"]["words"], 63741)
        self.assertEqual(instance["stats"]["segment_label"], "lines")

    def test_characters_are_empty_for_aeneid_build(self):
